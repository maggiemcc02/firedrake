...
# Pseudospectra of the 1D Poisson operator with Maggie's FULL inner loop:
...
# For the sinusoidal eigenfunctions of this problem the error is spread
# uniformly over (0, pi), so the honest prediction is that her marking will
# be broad (dofs roughly doubling per iteration) rather than localised --
# we record the marked fraction per refinement to verify it.
