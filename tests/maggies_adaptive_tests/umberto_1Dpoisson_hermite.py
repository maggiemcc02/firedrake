...
# Changing Umberto's code for pure poisson with Hermite elements
# Need to change all interpolates to project

epsilon = 0.25    # pseudospectral level, spec(A) = the integers
...
deg = 3           # Going to solve the folded problem with CG2 for both u and sigma
...
def make_problem(mesh):
    ...
    problem = LinearEigenproblem(A, M, bcs=bc, bc_shift=1e8, restrict=False)
...
def interval_mesh_from_vertices(xs):
    m = IntervalMesh(len(xs) - 1, 1.0) # make [0, 1] mesh 
...
class GoalAdaptive1DFoldedEigenSolver(GoalAdaptiveFoldedEigenSolver):
    ...
    #     except Exception:
    #         print(RED % f"WARNING - Issues with interpolating to new mesh - setting empty initial space")
    #         initial_space = []
    #     _space = tuple(initial_space)
def post_refinement(self):
    V_new = self.problem.output_space
    initial_space = []
    try:
        bc_new = DirichletBC(V_new, 0, "on_boundary")

        for old_u in self.vecs:
            new_u = Function(V_new)
            new_u.project(old_u)
            # Cross-mesh projection cannot impose BCs internally,
            # so enforce the homogeneous value BC afterwards.
            bc_new.apply(new_u)
            initial_space.append(new_u)

    except Exception as exc:
        print(RED % ("WARNING - Issues projecting to the new mesh; "f"setting empty initial space: {exc}"))
        initial_space = []
...
base_problem = make_problem(IntervalMesh(N0, pi)) # set the standard base problem to reuse
...
def dwr_solve(zval, hK):
    ...
    solver = GoalAdaptive1DFoldedEigenSolver(
        ...
        imag_tol=1e-12, mult_tol=1e-2, can_interp = False, solver_parameters=sp)
    solver.solve()
...
spec = np.array([n**2 for n in range(1, 12)])
...
def plot_progress(sweep, records, active, field_samples):
    ...
    fig.tight_layout()
    fig.savefig(f"umberto_fullrun_output/1D_poisson/outer_state_sweep{sweep}.pdf")
    plt.close(fig)
    ...
    if len(field_samples) > 3:
        ...
        fig.savefig(f"umberto_fullrun_output/1D_poisson/field_sweep{sweep}.pdf")
        plt.close(fig)
...
_write_triangulated_vtk(
    "umberto_fullrun_output/1D_poisson/poisson1d_dwr_local_pseudospectra.vtk",
    pts.real, pts.imag, tri_indices, residual_f)
# Write the eact eigenvalies
_write_eigenvalues_vtk(
    "umberto_fullrun_output/1D_poisson/poisson1d_dwr_local_eigenvalues.vtk",
    [complex(n) for n in spec if xmin <= n <= xmax])
print(GREEN % "wrote poisson1d_dwr_local VTK files")
...
for ext in ("png", "pdf"):
    fig.savefig(f"umberto_fullrun_output/1D_poisson/pseudospectra_field.{ext}",
                dpi=300)
plt.close(fig)
...
fig.tight_layout()
fig.savefig("umberto_fullrun_output/1D_poisson/pseudospectra_contour.pdf")
plt.close(fig)
