...
grid = np.append(np.arange(2.0, 4.0, h),[4.0]) 
smallest_eigvals = []
phi_vals = []
enriched_phi_vals = []
DWR_phi_vals = []
...
# exact distance to the spectrum
exact_dist = np.min(np.abs(np.subtract.outer(grid, exact_omega)), axis=1)
...
np.savez(
    f"{z_dir}/new_phi_plot_data_N={N}.npz",
    grid=np.array(grid),
    phi_vals=np.array(phi_vals),
    enriched_phi_vals=np.array(enriched_phi_vals),
    DWR_phi_vals=np.array(DWR_phi_vals),
    exact_omega=np.array(exact_omega),
...
plt.plot(grid, exact_dist, linestyle = '--', color = 'grey', label = r"Exact $\operatorname{dist}(z, \operatorname{Sp}(A))$")
...
plt.legend()
plt.savefig(f"{z_dir}/new_phi_plot_{N=}.pdf")
plt.close()