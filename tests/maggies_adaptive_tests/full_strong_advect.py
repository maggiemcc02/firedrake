...
# THE FOLDED ADVECTION DIFFUSION EXAMPLE 
##################################################################
...
def make_problem(mesh, fold):
    ...
    V = FunctionSpace(mesh, "Hermite", deg)
...
def _uniform_error_indicators(F, z_err, options):
    ...
    # Recover the physical mesh from the residual form
    v, = F.arguments()
    V = v.function_space()
    mesh = V.mesh().unique()

    # One value per physical cell
    DG0 = FunctionSpace(mesh, "DG", 0)

    # Positive indicator on every cell
    eta_cell = Function(DG0)
    eta_cell.assign(1.0)

    return eta_cell


# Override the localization routine used by the adaptive eigensolver
_maggie._compute_residual_indicators = _uniform_error_indicators
...
# Uniform refinement code (new)
# def uniformly_refine_interval_mesh(mesh, comm=LOCAL_COMM):
#     # Current vertices
#     old_xs = np.sort(np.unique(np.real(mesh.coordinates.dat.data_ro)))
#     # Midpoint of every cell
#     mids = 0.5 * (old_xs[:-1] + old_xs[1:])
#     # Split every cell into two
#     new_xs = np.sort(np.concatenate([old_xs, mids]))
#     return interval_mesh_from_vertices(new_xs,comm=comm)
...
class GoalAdaptive1DFoldedEigenSolver(GoalAdaptiveFoldedEigenSolver):
    ...
    def compute_efficiency_indices(self, eta_cell, eta_h,eta):
        # Localisation is deliberately disabled for this problem.
        pass
    ...
    # def post_refinement(self):
    #     # Pull the new Hermite space
    #     V_new = self.problem.output_space
    #     initial_space = []
    #     try:
    #         for old_u in self.vecs:
    #             new_u = Function(V_new)
    #             # Hermite does not support interpolation here,
    #             # so project onto the new mesh instead.
    #             new_u.project(old_u)
    #             initial_space.append(new_u)
    #     except Exception as err:
    #         PETSc.Sys.Print(
    #             RED % ("WARNING - Issues projecting to new mesh; "f"setting empty initial space. Error: {err}"))
    #         initial_space = []
    #     self.initial_space = tuple(initial_space)
...
def dwr_solve(zval, hK, fold):
    ...
    z.assign(zval)
    zbar.assign(np.conjugate(zval)) # Need zbar for our foldedproblem
...
# STARTING HERMITAIN TEST (Check fold)
##################################################################

ztest = 14.333 - 3.333j
z.assign(ztest)
zbar.assign(np.conjugate(ztest))
...
n = np.arange(1, 101)
full_spec = nu_value*(n*np.pi)**2 + 1/(4*nu_value)
spec = full_spec[ (full_spec >= xmin) & (full_spec <= xmax)]
