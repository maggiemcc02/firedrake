...
class GoalAdaptiveSolverBase:
    ...
    def should_refine(self, it, eta_h, eta):
        ...
        return if abs(eta_h) >= self.options.tolerance
    ...
    def step(self, it):
        ...
        #     raise StopIteration
        if not should_refine(self, it, eta_h, eta): # tolerance stopping criteria 
            self.print("Error estimate below tolerance, finished.")
            raise StopIteration
        elif it == self.options.max_it - 1: # max iter stopping criteria
            self.print(f"Maximum iteration ({self.options.max_it}) reached. Exiting.")
...
class GoalAdaptiveFoldedEigensolver(SteadyGoalAdaptiveSolver, OptionsManager):
    ...
    # 1. Maggie change - New init (made by chat to match the nonlinar solver approach)
    ...
    def should_refine(self, it, eta_h, eta):
        ...
        if check_up:
            print(GREEN % f'The L2 aligned enriched soln was very small (in norm) - red flag!!')
            print(GREEN % f'As a result, we will adapt the eigensolve')
            return True 
        ...
        if gap_spec * gap_correct < 0:
            print(GREEN % f'The corrected phi and original phi disagree')
            print(GREEN % f'As a result, we will adapt the eigensolve')
            return True 
        
        # Otherwise, dont adapt
        else:
            print(GREEN % f'The corrected and original phi match')
            print(GREEN % f'No need to do an adaptive solve')
            print(GREEN % f'But I will adapt anyways for now')
            return False 
